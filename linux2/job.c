...
void Output()
{
	...
	for (p = head; p != NULL; p = p->next)
	{
		printf(	"jid:\t%d\n"
			...
			"wait time:\t%d\n"
			"run time:\t%d\n"
			"state:\t%d (1 means READY, 2 means RUNNING, 3 means DONE) \n",
			p->job->jid,p->job->pid,p->job->defpri,p->job->curpri,p->job->ownerid,p->job->wait_time,p->job->run_time,(int)p->job->state);
